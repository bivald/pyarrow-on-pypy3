...
build-base:
	docker build -t pyarrow-on-pypy --no-cache .

build: ## Build docker image which includes the whl file
	@echo "Building docker container and wheel" \
	&& docker build -t pyarrow-on-pypy39 -f Dockerfile-from-base --no-cache .
...
test:
	docker run -v ${PWD}/run-tests.sh:/run-tests.sh --rm -it pyarrow-on-pypy39 bash /run-tests.sh
